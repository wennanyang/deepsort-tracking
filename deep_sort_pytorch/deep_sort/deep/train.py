...
parser = argparse.ArgumentParser(description="Train on plane39")
parser.add_argument("--data-dir", default='/home/ywn/reID/YOLOv5-DeepSort_Pytorch/mot_yolo_all/cropped', type=str)
...
transform_train = torchvision.transforms.Compose([
    torchvision.transforms.CenterCrop((128, 64)),
...
def test(epoch):
    ...
    if acc  > best_acc:
        best_acc = acc
        print("Saving parameters to checkpoint/ckpt_plane.t7")
        checkpoint = {
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(checkpoint, './checkpoint/ckpt_plane.t7')
    # torch.save(checkpoint, './checkpoint/ckpt.pt')
...
def main():
    for epoch in range(start_epoch, start_epoch+1000): #修改数字设置训练轮数
